...
class Board:
    ...
    def __init__(self, vertical, horizontal):
        self.board = self.set_board(vertical, horizontal)
        self.set_boarder()
        self.set_opening()
        self.recursive_divide(y=[0, self.board.shape[0]], x=[0, self.board.shape[1]])

    def set_board(self, vertical, horizontal):
        # defines the board
    ...
    def recursive_divide(self, y, x):
        ...
        if orientation == Orientation.VERTICAL:
            ...
            self.set_line(y, orientation, line)
            self.recursive_divide(y, [x[0], line + 1])
            self.recursive_divide(y, [line + 1, x[1]])
        else:
            line = random.randint(y[0], y[1] - 2)
            self.set_line(x, orientation, line)
            self.recursive_divide([y[0], line + 1], x)
            self.recursive_divide([line + 1, y[1]], x)

    def set_line(self, size, orientation, coordinate):
